...
if [ -z "$latest_wav" ]; then
  # Durante el arranque puede no existir ningun WAV todavia.
  # Docker start_period cubre el caso normal.
...
echo "OK: latest_wav=$(basename "$latest_wav") age=${age}s min_bytes=${min_bytes} disk=${used_percent}%"
exit 0
